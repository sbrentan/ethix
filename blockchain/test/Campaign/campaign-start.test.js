const { 
    prepareStartParams,
    startCampaign 
} = require("../helpers/campaign/start-helper.js");
const { assertAccountsValidity } = require("./contract-deployment.test.js").assertions;
const { log } = require("../../common/utils.js");
const { expect } = require("chai");

const assertCampaignStart = async (contract, params) => {
    const start_tx_outcome = await startCampaign(contract, params);
    await expect(start_tx_outcome.tx).to.emit(contract, "CampaignStarted");
    expect(start_tx_outcome.campaignId).to.match(/^0x[0-9a-fA-F]{64}$/);
}

const assertCampaignStartFailure = async (contract, params) => {
    const start_tx_outcome = await startCampaign(contract, params);
    await expect(start_tx_outcome.method).to.be.reverted;
    expect(start_tx_outcome.tx).to.be.null;
}

const test_request_fails_if_is_not_from_owner = async (contract, accounts) => {
    log();
    log(`[Test request fails if is not from owner]`, tabs = 2, sep = '');

    const { donor } = assertAccountsValidity(contract, accounts);

    const start_params = await prepareStartParams({ from: donor });

    await assertCampaignStartFailure(donor.contract, start_params);
}

const test_start_fails_if_is_not_from_donor = async (contract, accounts) => {
    log();
    log(`[Test campaign start fails if is not from donor]`, tabs = 2, sep = '');

    const { other } = assertAccountsValidity(contract, accounts);

    const start_params = await prepareStartParams({ from: other });

    await assertCampaignStartFailure(contract, start_params);
}

const test_campaign_start = async (contract, accounts) => {
    log();
    log(`[Test campaign start]`, tabs = 2, sep = '');

    const { donor } = assertAccountsValidity(contract, accounts);

    const start_params = await prepareStartParams({ from: donor });

    await assertCampaignStart(contract, start_params);
}

const test_start_fails_if_is_already_started = async (contract, accounts) => {
    log();
    log(`[Test campaign start fails if is already started/funded]`, tabs = 2, sep = '');

    const { donor } = assertAccountsValidity(contract, accounts);

    const start_params = await prepareStartParams({ from: donor });

    await assertCampaignStart(contract, start_params);
    await assertCampaignStartFailure(contract, start_params);
}   

module.exports = {
    assertions: {
        assertCampaignStart,
        assertCampaignStartFailure
    },
    tests: {
        test_request_fails_if_is_not_from_owner,
        test_start_fails_if_is_not_from_donor,
        test_campaign_start,
        test_start_fails_if_is_already_started
    }
}