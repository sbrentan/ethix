...
contract Charity {
    ...
    function isOrganizationVerified(address _organization) external view returns (bool) {
    ...
    function generateCampaignId(
        ...
        return keccak256(abi.encodePacked(_donor, _beneficiary, _title, campaignsIds.length));
    }
    ...
    function startCampaign(
        ...
        bytes32 campaignId = generateCampaignId(msg.sender, _beneficiary, _title);
    ...
    function getCampaign(bytes32 campaignId) external view returns(Campaign.CampaignDetails memory) {
        return campaigns[campaignId].getDetails();
    }

    function getCampaignTokens(bytes32 campaignId) external view returns(Campaign.Token[] memory) {
    ...
    function claimRefund(bytes32 campaignId) external {
        ...
        emit RefundClaimed(campaignId, msg.sender, campaigns[campaignId].getDetails().refunds);
    }

    function claimDonation(bytes32 campaignId) external {
        campaigns[campaignId].claimDonation(msg.sender);
        emit DonationClaimed(campaignId, msg.sender, campaigns[campaignId].getDetails().donations);
    }
    ...
    function getCampaignBalance(bytes32 campaignId) external view returns(uint256, uint256, uint256) {
