...
contract Campaign {
    ...
    function getTokens(
        ...
        returns (bytes32)
    {
        /*bytes32[] memory _tokens = new bytes32[](campaignDetails.tokensCount);

        for (uint i = 0; i < campaignDetails.tokensCount; i++) {
            _tokens[i] = _generateTokenId(campaignDetails.campaignId, i);
        }
    ...
    function isTokenValid(bytes32 t2_token, Signature calldata _signature) public view returns (bool) {
        ...
        if(!_signatureVerified(keccak256(abi.encodePacked(t2_token, campaignDetails.campaignId)), _signature.r, _signature.s, _signature.v, walletAddress)) {
            return false;
        }
